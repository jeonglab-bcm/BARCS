...
//
// These two functions are the inner loop of bbreg(): every IRLS iteration of
// every guide passes through them.  R-level fallbacks live in
// `.bb_wls_system()` and `.bb_wls_solve()` and remain the readable reference
// implementation; the compiled path is selected automatically whenever the
// shared library is loaded, which in an installed package is always.
